...
insert into users(log_in, pass_word)
values ('daulet', 'daulet1954kairat');
insert into users(log_in, pass_word)
values ('daulet2', 'daulet1955kairat');
insert into users(log_in, pass_word)
values ('daulet3', 'daulet1956kairat');

create or replace function check_password(login varchar(64), pass__word varchar(64)) returns bool as $$
    declare
        pswrd varchar(64);
    begin
        select pass_word from users where log_in = login into pswrd;
--         select pswrd;
        return pswrd = pass__word;
    end;
$$ language plpgsql;

select check_password('daulet', 'daulet1954kairat');
...
insert into some_upd(upd) values ('muratbek') returning *;
...
insert into person(f_name, l_name, bd) values ('daulet', 'Muratbek', '19-01-03') returning *;
...
insert into sales(total) values (100000) returning *;
...
create function stop() returns trigger as $$
    begin
...
$$ language plpgsql;
create trigger stop before delete on dd
    for each row execute procedure stop();
insert into dd (n, m)
values (10, 10);
...
call increase();
insert into tb1 values (1,'Daulet', '2003-01-19', 19, 50000, 16, 0);
select * from tb1;
...
insert into tb1 values ( 3, 'Daulet', '2002-01-19', 19, 40000, 20,0);
call increase40();
select * from tb1;
